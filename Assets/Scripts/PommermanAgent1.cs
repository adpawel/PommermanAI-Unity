...
public class PommermanAgent2v2 : Agent
{
...
    private const string SOLID_OBJECT_TAG = "WallSolid";
    ...
    private Bomb2v2 lastPlacedBomb;
    private bool bombPlacedByMe;
    private float prevDistanceFromBomb;
    public int stepCount = 0;
    private List<PommermanAgent2v2> enemies = new List<PommermanAgent2v2>();
    private List<PommermanAgent2v2> allies = new List<PommermanAgent2v2>();
...
        arenaManager = FindAnyObjectByType<ArenaManager2v2>();
        ...
        foreach (PommermanAgent2v2 pommermanAgent in Object.FindObjectsByType<PommermanAgent2v2>(FindObjectsSortMode.None))
        {
            if (pommermanAgent != this && pommermanAgent.teamId != this.teamId)
            {
                this.enemies.Add(pommermanAgent);
            }
            if (pommermanAgent != this && pommermanAgent.teamId == this.teamId)
            {
                this.allies.Add(pommermanAgent);
            }
        }
    }
    ...
    public void RefreshEnemies()
    {
        ...
        foreach (PommermanAgent2v2 pommermanAgent in Object.FindObjectsByType<PommermanAgent2v2>(FindObjectsSortMode.None))
        {
            if (pommermanAgent != this && pommermanAgent.teamId != this.teamId && pommermanAgent.isAlive)
            {
                this.enemies.Add(pommermanAgent);
            }
        }
    ...
    public void RefreshAllies()
    {
        ...
        foreach (PommermanAgent2v2 pommermanAgent in Object.FindObjectsByType<PommermanAgent2v2>(FindObjectsSortMode.None))
        {
            if (pommermanAgent != this && pommermanAgent.teamId == teamId && pommermanAgent.isAlive)
            {
                this.allies.Add(pommermanAgent);
            }
        }
...
    void FixedUpdate()
    {
        ...
        Collider[] nearby = Physics.OverlapSphere(transform.position, bombObserveRadius - 1);
        ...
        foreach (var c in nearby)
        {
            ...
            Bomb2v2 b = c.GetComponent<Bomb2v2>();
            if (b == null)
                continue;
    ...
    bool IsInBombBlast(Bomb2v2 bomb)
    {
    ...
    public void OnBombExploded(Bomb2v2 bomb)
    {
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        List<Bomb2v2> bombs = new List<Bomb2v2>();
        ...
        for (int i = 0; i < enemyObservationCount; i++)
        {
            if (i < allies.Count)
            {
                Vector3 delta = allies[i].transform.position - transform.position;
                float half = arenaManager != null ? arenaManager.size / 2f : 10f;

                sensor.AddObservation(Mathf.Clamp(delta.x / half, -1f, 1f));
                sensor.AddObservation(Mathf.Clamp(delta.z / half, -1f, 1f));
                sensor.AddObservation(
                    Mathf.Clamp01(Vector3.Distance(transform.position, allies[i].transform.position) / half)
                );
            }
            else
            {
    ...
    private bool IsBombThreatening(Bomb2v2 bomb)
    {
...
            arenaManager.OnAgentDied(this);
    ...
    private void PlaceBomb()
    {
        ...
        if (bombPrefab != null)
        {
            ...
            Bomb2v2 bombScript = newBomb.GetComponent<Bomb2v2>();
            if (bombScript != null)
            {
            ...
            for (int j = 1; j <= radius; j++)
            {
                ...
                foreach (Collider collider in Physics.OverlapBox(center, halfExtents, Quaternion.identity))
                {
                    if (collider.CompareTag("Player"))
                    {
                        ...
                        if (component != null && component.teamId != this.teamId)
                        {
...
    bool CanMoveTo(Vector3 position)
    ...
    public void SetArenaManager(ArenaManager2v2 manager)
    {
